...
export default {
  ...
  methods: {
    ...
    _loadMappingsForConcepts(concepts) {
      ...
      concepts = getItems(concepts.filter(concept => !(concept?.__MAPPED__ ?? []).find(item => this.$jskos.compareFast(item.registry, registry) && this.$jskos.compare(item.scheme, otherScheme))))
      ...
      if (otherScheme && conceptUris.length && registry) {
        Promise.all(_.chunk(conceptUris, 15).map(uris => this.getMappings({
          ...
          for (let concept of concepts.filter(c => !(c?.__MAPPED__ ?? []).find(item => this.$jskos.compareFast(item.registry, registry) && this.$jskos.compare(item.scheme, otherScheme)))) {
